...
<html>
  ...
  <!-- Facebook and Twitter integration -->
	...
	<div id="page">
					...
					<div class="row">
						...
						<div class="col-xs-10 text-right menu-1">
							<ul>
								...
								<li class="active"><a href="restaurant.php">热门餐厅</a></li>
		...
		<aside id="colorlib-hero">
			<div class="flexslider">
				<ul class="slides">
			   	<li style="background-image: url(XJB.jpg);">
	...
	if (!$cityName){
		...
		$result = mysqli_query($conn,"SELECT * FROM restaurant WHERE postalCode = {$array[$cityName]}");
	}
	$htmlDiv1 = <<<HTML
...
HTML;
    ...
    {
        $location =  "background-image: url(".$test["rstrPicture"].")";
        $htmlBlock = <<<HTML
        <div class="col-md-6 col-sm-6 animate-box">
                                <div class="hotel-entry">
                                    <a  class="hotel-img" style= '{$location}'>
                                        <p class="price"><span>{$test["rstrRating"]}</span><small> /10.0</small></p>
                                    </a>
                                    <div class="desc">
                                        <h3>{$test["rstrName"]}</h3>
                                        <h>{$test["rstrLocation"]}</h>
                                        <span class="place">{$test["bookingNumber"]}</span>
                                        <p>{$test["recommendDishes"]} </p>
                                    </div>
                                </div>
                            </div>
HTML;
