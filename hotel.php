...
<html>
  ...
  <!-- Facebook and Twitter integration -->
					...
					<div class="row">
						...
						<div class="col-xs-10 text-right menu-1">
							<ul>
								...
								<li class="active"><a href="hotels.html">酒店</a></li>
	...
	if (!$cityName){
		$result = mysqli_query($conn,"SELECT * FROM hotel h, room r WHERE h.hotelName = r.hotelName AND r.roomType = '双床房'");
	} else {
		$result = mysqli_query($conn,"SELECT * FROM hotel h, room r WHERE h.postalCode = {$array1[$cityName]} AND h.hotelName = r.hotelName AND r.roomType = '双床房'");
	}
	$htmlDiv1 = <<<HTML
...
HTML;
    ...
    {
		$location =  "background-image: url(".$test["hotelPicture"].")";
		$hotelName = $test["hotelName"];
        $htmlBlock = <<<HTML
        <div class="col-md-6 col-sm-6 animate-box">
                                <div class="hotel-entry">
                                    <a href="room.php?hotelName={$hotelName}" class="hotel-img" style= '{$location}'>
                                        <p class="price"><span>￥{$test["roomPrice"]}</span><small> /晚</small></p>
                                    </a>
                                    <div class="desc">
                                        <h>{$test["hotelRating"]}/5.0</h>
                                        <h3><a href="room.php">{$test["hotelName"]}</a></h3>
                                        <span class="place">{$array2[$test["postalCode"]]}</span>
                                        <p> {$test["hotelLocation"]}</p>
                                    </div>
                                </div>
                            </div>
HTML;
