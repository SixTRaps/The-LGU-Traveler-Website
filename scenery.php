...
<html>
  ...
  <!-- Facebook and Twitter integration -->
	...
	if (!$cityName){
		$result = mysqli_query($conn,"SELECT * FROM scenery");
	} else {
		$result = mysqli_query($conn,"SELECT * FROM scenery WHERE postalCode = {$array[$cityName]} ");
		$result2 = mysqli_query($conn,"SELECT * FROM weather WHERE postalCode = {$array[$cityName]} ");
	...
	<div class="colorlib-wrap">
		<div class="container">
			...
			<h2>今日{$cityName}天气</h2>
			<br>
			<h3>{$test2["weatherState"]}  气温：{$test2["temperature"]}℃</h3>
			<h3>AQI：{$test2["AQI"]}   湿度：{$test2["humidity"]}</h3>
			<h3>降雨率：{$test2["chanceOfRain"]}</h3>
            <div class="col-md-4" style="float:right">
				<a href="restaurant.php?cityName={$cityName}">
                	<input type="submit" name="submit" id="submit" value="热门餐厅" class="btn btn-primary btn-block">
				</a>
            </div>
        </div>
...
HTML;
    ...
    while($test = mysqli_fetch_array($result))
    {
        ...
        $location =  "background-image: url(".$test["sceneryPicture"].")";
        $htmlBlock = <<<HTML
                ...
                <div class="room-wrap">
                    <div class="row">
						...
						<div class="hotel-entry">
                            <a class="hotel-img" style='{$location}'>
                                <p class="price"><span style="color:#FFF">{$test["sceneryRating"]}</span><small> /5.0</small></p>
                            </a>
						</div>
                        </div>
                        <div class="col-md-6 col-sm-6">
                            <div class="desc">
                                <span class="day-tour"></span>
                                <h2>{$test["sceneryName"]}</h2>
                                <span class="day-tour">{$test["sceneryLocation"]}</span>
                                <p>{$test["briefIntroduction"]}</p>
                            </div>
                        </div>
                    </div>
                </div>
